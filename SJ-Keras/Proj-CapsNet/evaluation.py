...
def evaluate(saved_model_path,
    ...
    for x, y in train_data_loader:
        ...
        out_hist.fill(dname="train", y_true=y, y_pred=y_pred)
    ...
    for example in dijet_generator:
        ...
        out_hist.fill(
            dname="test_dijet",
            y_true=example["y"], y_pred=y_pred)
        heatmap.fill(
            y_true=example["y"], y_pred=y_pred,
            pt=example["pt"], eta=example["eta"])
    ...
    for x, y in test_zjet_loader:
        y_pred = model.predict_on_batch(x)
        roc_zjet.append(y_true=example["y"], y_pred=y_pred)
        out_hist.fill("test_zjet", y_true=example["y"], y_pred=y_pred)
