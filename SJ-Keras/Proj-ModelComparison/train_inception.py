...
def train():
    ...
    prser.add_argument("--num_a", type=int, default=1)
    prser.add_argument("--num_b", type=int, default=1)
    prser.add_argument("--num_c", type=int, default=1)

    args = parser.parse_args()

    if '{name}' in args.log_dir:
        args.log_dir = args.log_dir.format(
            name="{}".format(args.model.title()))
    ...
    logger.update({
        "x": ["image_chad_33", "image_chad_mult_33",
              "image_lepton_33", "image_lepton_mult_33",
              "image_nhad_33", "image_nhad_mult_33",
              "image_photon_33", "image_photon_mult_33",
        "x_shape": (8, 33, 33)})
